...
interface IDenLite {
    ...
    function startGame(uint256 srcChainId, uint256 numSleepers_, uint256[] calldata checkpoints) external;
    function setCrossChainFermentedJars(uint256[] calldata fermentedJarIds) external;

    function getSlumberParty() external returns (SlumberParty memory);
}
